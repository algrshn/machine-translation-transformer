...
google=[]    
with open("dataset/UNv1.0/other/google_fr_translation_of_first_100_sentences_from_val_set.txt") as f:
    for line in f:
        if(line.strip()!=""):
            google.append(line)
...
for i in range(len(google)):
    ...
    google_corpus.append(google[i].split())
    references_corpus.append([french_orig.split()])


score=bleu_score(candidate_corpus, references_corpus)
print("\n\n\nMy BLEU score: {0:1.4f}".format(score))

google_score=bleu_score(google_corpus, references_corpus)
print("\n\n\nGoogle translate BLEU score: {0:1.4f}".format(google_score))
